...
6. Given a gateway, when it boots, then enrollment is the **only** way it
   ...
   reachable control / invalid token cannot start — it has no other identity to
...
10. Given a revoked gateway, when an operator calls
    ...
    `CONTROL_GATEWAY_ENROLL_TOKEN` — a still-valid shared token otherwise
...
- **Datastore auth hardening** (Valkey ACL/TLS, Postgres role-split/TLS) —
...
- `internal/api/gateway_auth_handler.go` (new) — `EnrollGateway`: constant-time
  ...
  time.Minute)` in the `RateLimiters` bundle + interceptor allow-list entry +
  a public-procedure exemption (no JWT).
...
- A gateway-CRL client: poll `ControlService.GetCertificateRevocationList` over
  ...
  unloaded or stale, or the fingerprint is revoked — reject the gateway. Mirror
  the gateway's own `loadInitialCRL` fail-closed boot posture.
...
- **Ephemeral-identity revocation model.** Because identity is per-boot,
  ...
  *live* cert fingerprint — enforced on **both** the control-facing plane
  ...
  `CONTROL_GATEWAY_ENROLL_TOKEN`** — a shared token is the sole enrollment
...
- **Audit.** Enroll, renew, and revoke are all events → audit-logged.
...
- **Breaking (deliberate clean break).** The static `--tls-cert`/`--tls-key`
  ...
  with no reachable control / valid token will not boot. Called out in the
...
- **Control HA:** the advisory-lock fix is safe to deploy standalone (single
...
- `ca.IssueCertificateFromCSR`, `mtls.PeerClassURI`, `crl.Store`/`crl.Cache`,
  `store.TryWithAdvisoryLock` — the reused primitives.

...
<!-- docref: begin src=server:internal/api/idp_handler.go#IDPHandler.CreateIdentityProvider:fb82987f,server:internal/api/idp_handler.go#IDPHandler.UpdateIdentityProvider:1a6117dd,server:internal/api/idp_handler.go#IDPHandler.DeleteIdentityProvider:f417537b -->
...
<!-- docref: begin src=server:internal/crypto/crypto.go#Encryptor.Encrypt:03be5e41,server:internal/crypto/crypto.go#NewEncryptor:1f25da9e,server:internal/api/idp_handler.go#IDPHandler.idpToProto:694891bf -->
The client secret is encrypted at rest with AES-256-GCM under the server's 32-byte `CONTROL_ENCRYPTION_KEY` before it ever reaches the event store (stored with the `enc:v1:` prefix), and it is **never returned** in any RPC response — the proto conversion simply has no secret field.
<!-- docref: end -->

{% callout type="note" title="enc:v1 vs enc:v2" %}
<!-- docref: begin src=server:internal/crypto/crypto.go#Encryptor.EncryptWithContext:22fe1d32,server:internal/crypto/crypto.go#SecretAAD:4ff6bd49 -->
The encryptor also has an AAD-bound `enc:v2:` format that ties a ciphertext to its row context (`device|action|type`) so a database-level attacker cannot relocate a secret between rows. Today that binding is applied to device secrets (LUKS passphrases, LPS passwords); the IdP client secret uses the plain `enc:v1` format.
<!-- docref: end -->
{% /callout %}
...
<!-- docref: begin src=server:internal/api/sso_handler.go#SSOHandler.GetSSOLoginURL:b33e02b8,server:internal/idp/state.go#GenerateState:f5d67f7c,server:internal/idp/oidc.go#OIDCProvider.AuthCodeURL:aa9b2707 -->
...
<!-- docref: begin src=server:internal/api/sso_handler.go#SSOHandler.SSOCallback:47d74ef6,server:internal/store/postgres/auth_state.go#AuthState.Consume:164a0127,server:internal/idp/oidc.go#OIDCProvider.VerifyAndExtractClaims:abea947e -->
...
<!-- docref: begin src=server:internal/idp/linker.go#Linker.LinkOrCreate:2bd54846 -->
...
<!-- docref: begin src=server:internal/api/idp_handler.go#IDPHandler.CreateIdentityProvider:fb82987f -->
...
<!-- docref: begin src=sdk:proto/pm/v1/control.proto#IdentityProvider.trust_email_assertions:cb24a82d,server:internal/scim/users_create.go#Handler.createUser:69352edb -->
...
<!-- docref: begin src=server:internal/api/sso_handler.go#SSOHandler.SSOCallback:47d74ef6 -->

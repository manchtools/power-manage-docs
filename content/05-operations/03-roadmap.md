...
| Terminal admin RBAC model | Landed as *permissions*: `TerminalAdminLimited` / `TerminalAdminFull` (passwordless sudoers policies inside sessions) plus device-group-scoped grants ([terminal access](/security/terminal-access)). Preset terminal-admin *roles* are still not seeded — build them from the permissions. |
...
| Standalone `RevokeCertificate` RPC | The underlying machinery **shipped**: a Valkey-backed per-fingerprint CRL with a fail-closed gateway cache; device deletion and cert renewal both revoke ([mTLS](/security/mtls)). What's still missing is an operator-facing "revoke this cert without deleting the device" RPC. |
...
| Cryptographic checkpointing of the audit log | Off-host hash chains and signed periodic anchors. Stronger tamper-evidence than the database-trigger append-only guard that exists today. |

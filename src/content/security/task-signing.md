...
Every task that crosses the Asynq queue (Redis-backed) carries an HMAC over its payload. The producer signs before enqueuing; the consumer verifies before handling. A compromised Redis can't forge dispatches to agents or feed bogus execution events back to the control inbox.

This is the first of two defences on dispatched work. The HMAC catches Redis tampering. The per-action RSA signature on each dispatch ([mTLS and signed actions](/security/mtls)) catches gateway tampering. Either one alone would leave a gap; together they cover the whole path control → Redis → gateway → agent.
...
A task that ends up on a Redis queue without a valid HMAC is something the application code never produced. Either the key is wrong, or somebody is poking at Redis directly.
...
- After a Redis instance is replaced with one whose authentication you can't audit.

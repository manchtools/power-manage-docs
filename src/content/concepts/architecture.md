...
flowchart LR
    ...
    Control -.->|enqueue| Valkey[(Valkey<br/>Asynq + RediSearch)]
    Valkey -.->|dequeue| Gateway[Gateway<br/>no DB]
    Valkey -.->|dequeue| Indexer[Indexer<br/>search]
    Gateway -.->|bidi stream / mTLS| Agent[Agent<br/>mTLS]
    Gateway -->|InternalService proxy| Control
    Indexer -->|FT.CREATE / FT.SEARCH| Valkey
```

## Control server

The control server is the only thing that touches Postgres. It runs on `:8081` and hosts:
...
Multi-gateway topology is supported via Valkey self-registration: each gateway publishes its hostname into a Valkey set on boot, and Traefik routes by SNI. Traffic for an agent goes to whichever gateway holds its current stream.

## Indexer

The indexer is a stateless service that drains search-related events off Asynq and writes RediSearch indexes into Valkey. It also runs a periodic reconciliation against Postgres (default every 1h, configurable) to repair drift if a write got lost. Run zero workers if you don't need search, one for most fleets, or several if your search QPS gets serious.
